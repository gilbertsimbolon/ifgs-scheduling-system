    ...
    <div class="container-xxl flex-grow-1">
        ...
        <div class="card">
            ...
            <div class="card-header border-bottom py-3">
                <form action="{{ route('memberships.index') }}" method="GET" class="row g-2 align-items-center">
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <select name="status" id="status" class="form-select">
                                                ...
                                                <option value="expired"
                                                    {{ request('status') === 'expired' ? 'selected' : '' }}>Kadaluarsa
                                                </option>
                                    ...
                                    @forelse ($memberships as $membership)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="d-flex flex-column">
                                                    <span
                                                        class="fw-semibold">{{ $membership->product?->name ?? '-' }}</span>
                                                    <small
                                            ...
                                            <td>
                                                <span
                                                    class="fw-semibold text-success">{{ $membership->formatted_price }}</span>
                                            </td>
                                            <td>
                                            ...
                                            <td class="text-center">
                                                <div class="d-inline-flex gap-1">
                                                    ...
                                                    <button type="button" class="btn btn-sm btn-icon btn-outline-info"
                                                        ...
                                                        data-created="{{ $membership->created_at ? $membership->created_at->format('d M Y, H:i') : '-' }}">
                                                    ...
                                                    <button type="button" class="btn btn-sm btn-icon btn-outline-warning"
                                                        ...
                                                        data-member-name="{{ $membership->member?->user?->name ?? '-' }} ({{ $membership->member?->member_code ?? '-' }})"
                                                        data-product-id="{{ $membership->product_id }}"
                                                        data-start-date="{{ $membership->start_date ? $membership->start_date->format('Y-m-d') : '' }}"
                                                        data-end-date="{{ $membership->end_date ? $membership->end_date->format('Y-m-d') : '' }}"
                                                        data-price="{{ (int) $membership->price }}"
                                                        data-status="{{ $membership->status }}">
                                                        <i class="bx bx-edit-alt"></i>
                                                    </button>

                                                    <!-- Batalkan Button (only if active) -->
                                                    @if ($membership->status === \App\Models\Membership::STATUS_ACTIVE)
                                                        <button type="button"
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="7" class="text-center py-5">
                                                <div
                                                    class="d-flex flex-column align-items-center justify-content-center w-100 py-3">
                                                    <div class="mb-2">
                                                        <i class="bx bx-credit-card fs-1 text-secondary"></i>
                                                    </div>
                                                    <h6 class="fw-semibold mb-1">Belum ada data membership</h6>
                                                    <span class="text-muted">Klik tombol "Tambah Membership" untuk
                                                        mendaftarkan transaksi pertama.</span>
                                                    ...
                                                    <span class="text-muted">
                                                        ...
                                                        @else
                                                            Klik tombol "Tambah Membership" untuk mendaftarkan transaksi
                                                            pertama.
                                                        @endif
                                                    </span>
                                                </div>
