...
@section('content')
    ...
    <div class="card p-3 mb-3 bg-white border text-center position-relative">
        <div class="mb-3 d-flex justify-content-center">
            <div class="position-relative d-inline-block">
                ...
                <label for="memberAvatarInput"
                    class="m-0 p-0 rounded-circle d-block cursor-pointer position-relative shadow-sm"
                    title="Klik untuk memilih atau mengubah foto profil" style="cursor: pointer;">
                    @if ($user->avatar_url)
                        <img src="{{ $user->avatar_url }}" alt="{{ $user->name }}"
                            class="rounded-circle object-fit-cover border border-2 border-primary" width="82"
                            height="82">
                    @else
                ...
                @if ($user->avatar_url)
                    <form action="{{ route('member.profil.avatar.delete') }}" method="POST"
                ...
                <form id="formUploadAvatar" action="{{ route('member.profil.avatar') }}" method="POST"
                    enctype="multipart/form-data" class="d-none">
                    @csrf
                    @method('PUT')
                    <input type="file" id="memberAvatarInput" name="avatar"
    ...
    <div class="card p-3 mb-3 bg-white border text-center">
        ...
        <div class="d-flex align-items-center justify-content-center gap-2">
            @if (Route::has('user.qr-code.download'))
                <a href="{{ route('user.qr-code.download') }}" class="btn btn-outline-primary btn-sm px-3 rounded-pill"
                    style="font-size: 0.75rem;">
                    <i class="bx bx-download me-1"></i> Simpan QR
                </a>
            @endif
            @if (Route::has('user.card'))
                <a href="{{ route('user.card', $user->slug) }}" target="_blank"
                    class="btn btn-outline-secondary btn-sm px-3 rounded-pill" style="font-size: 0.75rem;">
    ...
    <div class="card mb-3">
        ...
        <div class="card-body p-3">
            ...
            <div class="d-flex align-items-center justify-content-between py-2 border-bottom">
                ...
                <span
                    class="fw-semibold text-primary small">{{ $activeMembership->product?->name ?? 'Belum Berlangganan' }}</span>
            </div>
            <div class="d-flex align-items-center justify-content-between py-2">
                <span class="text-muted small" style="font-size: 0.78rem;">Terdaftar Sejak</span>
                <span
                    class="fw-semibold text-dark small">{{ $user->created_at ? $user->created_at->translatedFormat('d F Y') : '-' }}</span>
            </div>
        </div>
    </div>
    ...
    <div class="card mb-4 bg-white border">
        <div class="card-header py-2 px-3 bg-white border-bottom">
            <h6 class="mb-0 fw-bold text-dark fs-6 d-flex align-items-center gap-2">
                <i class="bx bx-cog text-primary"></i> Pengaturan Akun
            </h6>
        </div>
        <div class="list-group list-group-flush rounded-bottom">
            <button type="button"
                class="list-group-item list-group-item-action d-flex align-items-center justify-content-between py-3 px-3 border-bottom text-decoration-none bg-transparent"
            ...
            <button type="button"
                class="list-group-item list-group-item-action d-flex align-items-center justify-content-between py-3 px-3 text-decoration-none bg-transparent"
    ...
    <!-- Modal Edit Profil -->
    <div class="modal fade" id="modalEditProfil" tabindex="-1" aria-labelledby="modalEditProfilLabel"
        aria-hidden="true">
            ...
            <div class="modal-content border-0 shadow">
                ...
                <form action="{{ route('member.profil.update') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body p-3">
                        <div class="mb-3">
                            <label for="prof_name" class="form-label small fw-bold text-dark">Nama Lengkap <span
                                    class="text-danger">*</span></label>
                            <input type="text" class="form-control form-control-sm" id="prof_name" name="name"
                                value="{{ old('name', $user->name) }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="prof_email" class="form-label small fw-bold text-dark">Alamat Email <span
                                    class="text-danger">*</span></label>
                            <input type="email" class="form-control form-control-sm" id="prof_email" name="email"
                                value="{{ old('email', $user->email) }}" required>
                        </div>

                        <div class="mb-3">
                            <label for="prof_phone" class="form-label small fw-bold text-dark">Nomor Handphone /
                                WhatsApp</label>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-0 shadow">
                ...
                <form action="{{ route('member.profil.password') }}" method="POST">
                    ...
                    <div class="modal-body p-3">
                        <div class="mb-3">
                            <label for="current_password" class="form-label small fw-bold text-dark">Password Saat Ini
                        ...
                        <div class="mb-3">
                            <label for="new_password" class="form-label small fw-bold text-dark">Password Baru <span
                                    class="text-danger">*</span></label>
                        ...
                        <div class="mb-2">
                            <label for="new_password_confirmation" class="form-label small fw-bold text-dark">Konfirmasi
